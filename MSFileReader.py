...
from ctypes import byref, cast, c_long, c_double, POINTER
...
if __name__ == "__main__":
    ...
    with open(rawfile.GetFileName()+'.tsv', 'wt') as f:  
        for scan_number in range(1,rawfile.GetNumSpectra()+1):
            ...
            #clearedresultTime = pattern.sub('', splittedresult[0]) #might be useful for GetChroData
            clearedresultIntensity = pattern.sub('', splittedresult[1])
            clearedresultIntensity = clearedresultIntensity[1:]
            tic = sum(float(i) for i in clearedresultIntensity.split(','))
            #final = clearedresultTime+'\n'+clearedresultIntensity #might be useful is GetChroData is used
        
            print(str(tic) , file=f)
